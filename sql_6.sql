...
SELECT name FROM actor JOIN casting ON actor.id = casting.actorid WHERE movieid = 10522;

-- List the films in which 'Harrison Ford' has appeared
SELECT title FROM movie JOIN casting ON movieid= id WHERE actorid = 2216;

-- List the films where 'Harrison Ford' has appeared - but not in the starring role
SELECT title FROM movie JOIN casting ON movieid= id WHERE actorid = 2216 AND ord <> 1;
